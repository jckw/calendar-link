...
describe("Calendar Links", () => {
  describe("Google", () => {
    ...
    test("generate a google link that repeats annually", () => {
      const event: CalendarEvent = {
        title: "My Birthday",
        start: "1997-12-29",
        allDay: true,
      ...
      const expectedDates = encodeURIComponent(`${sTime}/${eTime}`);
      expect(link).toBe(
        `https://calendar.google.com/calendar/render?action=TEMPLATE&dates=${expectedDates}&recur=RRULE%3AFREQ%3DYEARLY&text=My%20Birthday`
      );
    });
  });
  ...
  test("should generate a repeating all-day event", () => {
    const event: CalendarEvent = {
      title: "My Birthday",
      start: "2019-12-29",
      allDay: true,
    ...
    const link = ics(event);
    expect(link).toBe(
      `data:text/calendar;charset=utf8,BEGIN:VCALENDAR%0AVERSION:2.0%0ABEGIN:VEVENT%0ADTSTART:20191229%0ADTEND:20191230%0ASUMMARY:My%20Birthday%0ARRULE:FREQ%3DYEARLY%0AEND:VEVENT%0AEND:VCALENDAR%0A`
    );
  });
});
